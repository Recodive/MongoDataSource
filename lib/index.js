...
class MongoDataSource extends apollo_datasource_1.DataSource {
    ...
    async find(fields = {}, options = {
        ...
        var _a;
        const cacheKey = this.getCacheKey("find", fields, options, sort, skip, limit), cacheDoc = await ((_a = this.cache) === null || _a === void 0 ? void 0 : _a.get(cacheKey));
        if (cacheDoc)
            return JSON.parse(cacheDoc);
        const docs = await this.antiSpam(cacheKey, async () => {
            ...
            if (sort)
                cursor.sort(sort.sort, sort.direction);
            if (skip)
                cursor.skip(skip);
            if (limit)
                cursor.limit(limit);
    ...
    getCacheKey(type, fields, options = {}, sort, skip, limit) {
